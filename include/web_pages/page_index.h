...
<div id="Keys" class="tab-content" style="display:block;">
    ...
    <div class="form-container">
        ...
        <div style="margin-top: 15px; padding: 12px; background: rgba(155,89,182,0.1); border: 1px solid rgba(155,89,182,0.3); border-radius: 6px;">
            <p style="font-size: 0.9em; color: #bbb; margin-bottom: 10px;" data-i18n="settings.restricted.description">
                Export or import both TOTP keys and passwords through an isolated, closed access-point session (no admin login required, gated by device PIN).
            </p>
            <button class="enter-restricted-mode-btn button user-activity" style="width:100%;" data-i18n="settings.restricted.btn">Enter Secure Import/Export Mode (Keys + Passwords)</button>
        </div>
    </div>
</div>
...
<div id="Passwords" class="tab-content">
    ...
    <div class="form-container">
        ...
        <form id="add-password-form">
            ...
            <label class="send-login-wrap" style="display:flex;align-items:center;gap:8px;margin-top:8px;font-size:0.85em;opacity:0.85;cursor:pointer;">
              <input type="checkbox" id="add-send-login" name="send_login" value="1" style="width:16px;height:16px;accent-color:var(--accent);">
              <span data-i18n="passwords.send_login.label">Send login before password</span>
            </label>
            <label class="wildcard-wrap" style="display:flex;align-items:center;gap:8px;margin-top:8px;font-size:0.85em;opacity:0.85;cursor:pointer;">
              <input type="checkbox" id="add-wildcard" name="wildcard" value="1" style="width:16px;height:16px;accent-color:var(--accent);" onchange="onAddWildcardToggle(this)">
              <span data-i18n="passwords.wildcard.label">Password Wildcard (random on-device password)</span>
            </label>
            <div id="add-login-fields" style="display:none;margin-top:8px;">
              <input type="text" id="add-login" name="login" placeholder="Login" data-i18n-placeholder="passwords.login.placeholder" style="margin-bottom:8px;">
              <select id="add-nav-mode" name="nav_mode" style="width:100%;padding:8px;border-radius:5px;border:1px solid #ddd;">
                <option value="enter">Enter</option>
                <option value="tab">Tab</option>
              </select>
              <div id="add-login-delay-wrap" style="display:none;margin-top:8px;">
                <label data-i18n="passwords.login_delay.label">Delay before password (ms)</label>
                <input type="number" id="add-login-delay" value="300" min="0" max="5000" step="50">
              </div>
            </div>
...
<div id="restricted-mode-modal" class="modal">
    <div class="modal-content">
        <span class="close" onclick="closeRestrictedModeModal()">&times;</span>
        <h3 data-i18n="settings.restricted.modal.title">Enter Secure Import/Export Mode</h3>
        <p data-i18n="settings.restricted.modal.desc">Device will reboot into an isolated import/export session over its own closed WiFi access point. The web cabinet session will end. Enter your admin password to confirm.</p>
        <div class="form-group">
            <label for="restricted-mode-password" data-i18n="settings.restricted.modal.password">Admin Password:</label>
            <div style="position:relative;">
                <input type="password" id="restricted-mode-password" style="width:100%;padding-right:40px;box-sizing:border-box;" class="user-activity">
                <button type="button" onclick="toggleRestrictedModePassword()" style="position:absolute;right:6px;top:50%;transform:translateY(-50%);background:none;border:none;cursor:pointer;font-size:18px;color:#aaa;padding:0;" title="Show/hide password">👁️</button>
            </div>
        </div>
        <button id="restricted-mode-submit-btn" class="button user-activity" style="background-color:#e74c3c;" onclick="submitEnterRestrictedMode()" data-i18n="settings.restricted.modal.confirm">Reboot into Secure Mode</button>
    </div>
</div>
...
<div id="password-edit-modal" class="modal">
    <div class="modal-content">
        ...
        <div class="form-group">
            ...
            <label class="send-login-wrap" style="display:flex;align-items:center;gap:8px;margin-top:8px;font-size:0.85em;opacity:0.85;cursor:pointer;">
              <input type="checkbox" id="edit-send-login" name="send_login" value="1" style="width:16px;height:16px;accent-color:var(--accent);">
              <span data-i18n="passwords.send_login.label">Send login before password</span>
            </label>
            <div id="edit-login-fields" style="display:none;margin-top:8px;">
              <input type="text" id="edit-login" name="login" placeholder="Login" data-i18n-placeholder="passwords.login.placeholder" style="margin-bottom:8px;">
              <select id="edit-nav-mode" name="nav_mode" style="width:100%;padding:8px;border-radius:5px;border:1px solid #ddd;">
                <option value="enter">Enter</option>
                <option value="tab">Tab</option>
              </select>
              <div id="edit-login-delay-wrap" style="display:none;margin-top:8px;">
                <label data-i18n="passwords.login_delay.label">Delay before password (ms)</label>
                <input type="number" id="edit-login-delay" value="300" min="0" max="5000" step="50">
              </div>
            </div>
            <div id="edit-wildcard-len-wrap" style="display:none;margin-top:8px;">
                <label for="edit-wildcard-len" data-i18n="passwords.wildcard.length.label">Generated password length</label>
                <input type="number" id="edit-wildcard-len" value="16" min="1" max="64" style="width:80px;">
            </div>
            <div class="password-strength-container">
                <div class="password-strength-bar">
...
function formatPasswordMetadata(pwd, allPasswords) {
    ...
    if (pwd.send_login) {
        parts.push('<span class="badge" title="' + 
            tr('passwords.send_login.hint') + '">LOGIN' +
            (pwd.nav_mode === 'tab' ? ' &#8677;' : ' &#x23CE;') + '</span>');
    }
...
// Toggle login delay field visibility based on nav_mode
function updateLoginDelayVisibility(navModeSelectId, delayWrapId) {
    const navModeSelect = document.getElementById(navModeSelectId);
    const delayWrap = document.getElementById(delayWrapId);
    if (navModeSelect && delayWrap) {
        const navMode = navModeSelect.value;
        delayWrap.style.display = (navMode === 'enter') ? 'block' : 'none';
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Toggle login fields visibility for add form
    const addSendLogin = document.getElementById('add-send-login');
    const addLoginFields = document.getElementById('add-login-fields');
    if (addSendLogin && addLoginFields) {
        addSendLogin.addEventListener('change', function() {
            addLoginFields.style.display = this.checked ? 'block' : 'none';
        });
        // Initialize visibility on page load based on checkbox state
        addLoginFields.style.display = addSendLogin.checked ? 'block' : 'none';
    }
    
    // Toggle login fields visibility for edit form
    const editSendLogin = document.getElementById('edit-send-login');
    const editLoginFields = document.getElementById('edit-login-fields');
    if (editSendLogin && editLoginFields) {
        editSendLogin.addEventListener('change', function() {
            editLoginFields.style.display = this.checked ? 'block' : 'none';
        });
        // Initialize visibility on page load based on checkbox state
        editLoginFields.style.display = editSendLogin.checked ? 'block' : 'none';
    }
    
    // Add nav-mode change handlers
    const addNavMode = document.getElementById('add-nav-mode');
    if (addNavMode) {
        addNavMode.addEventListener('change', function() {
            updateLoginDelayVisibility('add-nav-mode', 'add-login-delay-wrap');
        });
        // Initial state
        updateLoginDelayVisibility('add-nav-mode', 'add-login-delay-wrap');
    }
    
    const editNavMode = document.getElementById('edit-nav-mode');
    if (editNavMode) {
        editNavMode.addEventListener('change', function() {
            updateLoginDelayVisibility('edit-nav-mode', 'edit-login-delay-wrap');
        });
    }
...
function editPassword(index) {
    ...
    if (passwordsData[index].wildcard) {
        // System-managed entry: never fetch/display a real password, and
        // never show fields that don't apply (category, auto-send, send-login,
        // generate, reveal, strength) — only the generated length is editable.
        document.getElementById('edit-password-name').value = 'random';
        document.getElementById('edit-password-name').disabled = true;
        document.getElementById('edit-password-value').value = '';
        document.getElementById('edit-password-value').disabled = true;
        document.getElementById('edit-password-value').placeholder = 'Generated on device, not stored here';
        document.querySelector('#password-edit-modal .password-generate').style.display = 'none';
        document.querySelector('#password-edit-modal .password-toggle').style.display = 'none';
        document.querySelector('#password-edit-modal .cat-selector-wrap').style.display = 'none';
        document.querySelector('#password-edit-modal .auto-send-wrap').style.display = 'none';
        document.querySelector('#password-edit-modal .send-login-wrap').style.display = 'none';
        document.getElementById('edit-login-fields').style.display = 'none';
        document.querySelector('#password-edit-modal .password-strength-container').style.display = 'none';
        document.getElementById('edit-wildcard-len-wrap').style.display = 'block';
        document.getElementById('edit-wildcard-len').value = passwordsData[index].wildcard_len || 16;
        openPasswordEditModal();
        return;
    }
    document.getElementById('edit-password-name').disabled = false;
    document.getElementById('edit-password-value').disabled = false;
    document.getElementById('edit-password-value').placeholder = '';
    document.getElementById('edit-wildcard-len-wrap').style.display = 'none';
    document.querySelector('#password-edit-modal .password-generate').style.display = '';
    document.querySelector('#password-edit-modal .password-toggle').style.display = '';
    document.querySelector('#password-edit-modal .cat-selector-wrap').style.display = '';
    document.querySelector('#password-edit-modal .auto-send-wrap').style.display = '';
    document.querySelector('#password-edit-modal .send-login-wrap').style.display = '';
    document.querySelector('#password-edit-modal .password-strength-container').style.display = '';
    ...
    makeAuthenticatedRequest('/api/passwords/get', { method: 'POST', body: new URLSearchParams(formData) })
        .then(async response => {
            ...
            document.getElementById('edit-send-login').checked = data.send_login || false;
            document.getElementById('edit-login').value = data.login || '';
            document.getElementById('edit-nav-mode').value = data.nav_mode || 'enter';
            document.getElementById('edit-login-delay').value = data.login_delay_ms || 300;
            document.getElementById('edit-login-fields').style.display = data.send_login ? 'block' : 'none';
            updateLoginDelayVisibility('edit-nav-mode', 'edit-login-delay-wrap');
            updateEditCatBtn();
...
function savePasswordEdit() {
    ...
    if (passwordsData && passwordsData[currentEditIndex] && passwordsData[currentEditIndex].wildcard) {
        const wildcardLen = document.getElementById('edit-wildcard-len').value || 16;
        const formData = new FormData();
        formData.append('index', currentEditIndex);
        formData.append('wildcard_len', wildcardLen);
    ...
    if (document.getElementById('edit-send-login').checked) {
        formData.append('send_login', '1');
    }
    formData.append('login', document.getElementById('edit-login').value || '');
    formData.append('nav_mode', document.getElementById('edit-nav-mode').value || 'enter');
    formData.append('login_delay_ms', document.getElementById('edit-login-delay').value || '300');
...
async function quickUpdateCategory(pwdIndex, newCat, badgeEl) {
    ...
    if (!pwd) return;
    if (pwd.wildcard) {
        showStatus(tr('passwords.wildcard.no_copy'), true);
        return;
    }
    ...
    formData.append('login', pwd.login || '');
    formData.append('nav_mode', pwd.nav_mode || 'enter');
    formData.append('login_delay_ms', pwd.login_delay_ms || '300');
...
function fetchPasswords(){
    ...
    if (cachedPasswords) {
        ...
        // Нормализуем формат данных из кэша (на случай старой версии или неполных полей)
        passwordsData = cachedPasswords.map(pwd => ({
            name: pwd.name,
            ...
            send_login: pwd.send_login || false,
            login: pwd.login || '',
            nav_mode: pwd.nav_mode || 'enter',
            login_delay_ms: pwd.login_delay_ms || 300,
            username: pwd.username || '',
            url: pwd.url || '',
            wildcard: pwd.wildcard || false,
            wildcard_len: pwd.wildcard_len || 16
        }));
        ...
        visiblePasswords.forEach((password, visibleIndex) => {
            ...
            row.innerHTML = '<td><span class="drag-handle">::</span></td><td>' + 
                            catBadgeHTML(password.category || '', !password.wildcard) + 
                            '<br>' + password.name + 
                            (password.wildcard ? ' <span class="key-badges"><span class="key-badge"><span class="badge">🎲 ' + tr('passwords.wildcard.badge') + '</span></span></span>' : '') +
                            formatPasswordMetadata(password, passwordsData) + 
                            '</td><td><button class="button-copy user-activity" ' + (password.wildcard ? 'disabled style="opacity:0.4;cursor:not-allowed;" title="' + tr('passwords.wildcard.no_copy') + '"' : 'onclick="copyPassword(' + originalIndex + ')"') + '>' + tr('passwords.btn.copy') + '</button><button class="button-action user-activity" onclick="editPassword(' + originalIndex + ')">' + tr('passwords.btn.edit') + '</button><button class="button-delete user-activity" onclick="removePassword(' + originalIndex + ')">' + tr('passwords.btn.remove') + '</button></td>';
        });
    ...
    .then(async response => {
        ...
        visiblePasswords.forEach((password, visibleIndex) => {
            ...
            row.innerHTML = '<td><span class="drag-handle">::</span></td><td>' + 
                            ...
                            '<br>' + password.name + 
                            (password.wildcard ? ' <span class="key-badges"><span class="key-badge"><span class="badge">🎲 ' + tr('passwords.wildcard.badge') + '</span></span></span>' : '') +
                            formatPasswordMetadata(data[originalIndex], data) + 
                            '</td><td><button class="button-copy user-activity" ' + (password.wildcard ? 'disabled style="opacity:0.4;cursor:not-allowed;" title="' + tr('passwords.wildcard.no_copy') + '"' : 'onclick="copyPassword(' + originalIndex + ')"') + '>' + tr('passwords.btn.copy') + '</button><button class="button-action user-activity" onclick="editPassword(' + originalIndex + ')">' + tr('passwords.btn.edit') + '</button><button class="button-delete user-activity" onclick="removePassword(' + originalIndex + ')">' + tr('passwords.btn.remove') + '</button></td>';
        });
        initializeDragAndDrop('passwords-table', 'passwords');
    })
...
function onAddWildcardToggle(checkbox) {
    const nameField = document.getElementById('password-name');
    const passwordField = document.getElementById('password-value');
    const generateBtn = document.querySelector('#add-password-form .password-generate');
    const catWrap = document.querySelector('#add-password-form .cat-selector-wrap');
    const autoSendWrap = document.querySelector('#add-password-form .auto-send-wrap');
    const sendLoginWrap = document.querySelector('#add-password-form .send-login-wrap');
    if (checkbox.checked) {
        const alreadyExists = (passwordsData || []).some(p => p.wildcard);
        if (alreadyExists) {
            checkbox.checked = false;
            showStatus(tr('passwords.wildcard.exists'), true);
            return;
        }
        if (generateBtn) generateBtn.style.pointerEvents = 'none';
        if (generateBtn) generateBtn.style.opacity = '0.3';
        nameField.disabled = true;
        passwordField.disabled = true;
        nameField.value = 'random';
        passwordField.value = '';
        passwordField.placeholder = 'Generated on device, not stored here';
        if (catWrap) catWrap.style.display = 'none';
        if (autoSendWrap) autoSendWrap.style.display = 'none';
        if (sendLoginWrap) sendLoginWrap.style.display = 'none';
        document.getElementById('add-auto-send').checked = false;
        document.getElementById('add-send-login').checked = false;
        document.getElementById('add-login-fields').style.display = 'none';
        _addCatValue = '';
        updateAddCatBtn();
    } else {
        if (generateBtn) generateBtn.style.pointerEvents = '';
        if (generateBtn) generateBtn.style.opacity = '';
        nameField.disabled = false;
        passwordField.disabled = false;
        nameField.value = '';
        passwordField.placeholder = '';
        if (catWrap) catWrap.style.display = '';
        if (autoSendWrap) autoSendWrap.style.display = '';
        if (sendLoginWrap) sendLoginWrap.style.display = '';
    }
}

document.getElementById('add-password-form').addEventListener('submit', function(e) {
    e.preventDefault();
    const isWildcard = document.getElementById('add-wildcard').checked;
    const name = isWildcard ? 'random' : document.getElementById('password-name').value;
    const password = isWildcard ? 'placeholder' : document.getElementById('password-value').value;
    const formData = new FormData();
    formData.append('name', name);
    formData.append('password', password);
    ...
    if (document.getElementById('add-send-login').checked) {
        formData.append('send_login', '1');
    }
    formData.append('login', document.getElementById('add-login').value || '');
    formData.append('nav_mode', document.getElementById('add-nav-mode').value || 'enter');
    formData.append('login_delay_ms', document.getElementById('add-login-delay').value || '300');
    if (isWildcard) {
        formData.append('wildcard', '1');
    }
    ...
    makeAuthenticatedRequest('/api/passwords/add', {method: 'POST', body: formData})
        .then(data => {
            ...
            _addCatValue = '';
            updateAddCatBtn();
            document.getElementById('password-name').disabled = false;
            document.getElementById('password-value').disabled = false;
            onAddWildcardToggle(document.getElementById('add-wildcard'));
            // Hide login fields after form reset (they should be hidden by default)
            document.getElementById('add-login-fields').style.display = 'none';
        })
        .catch(err => showStatus('Error: ' + err, true));
});
...
async function copyPassword(index) {
    if (passwordsData && passwordsData[index] && passwordsData[index].wildcard) {
        showStatus(tr('passwords.wildcard.no_copy'), true);
        return;
    }
...
async function fetchHiddenSpaceStatus() {
    try {
        ...
        if (data.current_space === 'B') {
            ...
            // Check if PIN is enabled - Hidden Space requires PIN encryption
            const cachedPin = CacheManager.get('pin_settings');
            const pinEnabled = cachedPin ? cachedPin.enabledForDevice : true;
            
            if (!pinEnabled) {
                // Grey out Hidden Space section when PIN is disabled
                toggle.disabled = true;
                btn.disabled = true;
                hint.innerHTML = '<span style="color:#ff6b6b;">⚠ Hidden Space requires Startup PIN to be enabled first.</span>';
                btn.style.opacity = '0.5';
                btn.style.cursor = 'not-allowed';
            } else {
                // PIN is enabled - restore normal functionality
                btn.disabled = false;
                btn.style.opacity = '1';
        ...
        // ═══ DISABLE PIN TOGGLE WHEN HIDDEN SPACE EXISTS ═══
        // PIN cannot be disabled while Hidden Space is active
        const pinToggle = document.getElementById('pin-enabled-device');
        if (pinToggle) {
            if (data.current_space === 'A' && data.hidden_space_enabled) {
                pinToggle.disabled = true;
                pinToggle.title = 'Cannot disable PIN while Hidden Space exists';
            } else {
        ...
        // ═══ GREY OUT HIDDEN SPACE SECTION WHEN STARTUP PIN IS DISABLED ═══
        // Hidden Space's dual-slot format requires an existing PIN-encrypted
        // device key. Creating it without startup PIN corrupts the key file.
        {
            const cachedPinForHS = CacheManager.get('pin_settings');
            const startupPinEnabled = cachedPinForHS ? !!cachedPinForHS.enabledForDevice : true;
            const hsSection = document.getElementById('hidden-space-section');
            const hsToggle = document.getElementById('hidden-space-enabled');
            const hsBtn = document.getElementById('hidden-space-action-btn');
            if (hsSection) {
                if (!startupPinEnabled && data.current_space === 'A' && !data.hidden_space_enabled) {
                    hsSection.style.opacity = '0.4';
                    hsSection.style.pointerEvents = 'none';
                    hsSection.title = 'Enable Startup PIN first to use Hidden Space';
                    if (hsToggle) hsToggle.disabled = true;
                    if (hsBtn) hsBtn.disabled = true;
                } else {
                    hsSection.style.opacity = '';
                    hsSection.style.pointerEvents = '';
                    hsSection.title = '';
                    if (hsToggle) hsToggle.disabled = false;
                    if (hsBtn) hsBtn.disabled = false;
                }
            }
        }

        // ═══ GREY OUT DURESS PIN SECTION WHEN STARTUP PIN IS DISABLED ═══
        // Duress PIN only makes sense as an alternate entry on the startup
        // PIN lock screen — with startup PIN off there is no entry point
        // that could ever trigger it.
        {
            const cachedPinForDuress = CacheManager.get('pin_settings');
            const startupPinEnabledDuress = cachedPinForDuress ? !!cachedPinForDuress.enabledForDevice : true;
            const duressSection = document.getElementById('duress-pin-section');
            const duressToggle = document.getElementById('duress-pin-enabled');
            if (duressSection) {
                if (!startupPinEnabledDuress) {
                    duressSection.style.opacity = '0.4';
                    duressSection.style.pointerEvents = 'none';
                    duressSection.title = 'Enable Startup PIN first to use Duress PIN';
                    if (duressToggle) duressToggle.disabled = true;
                } else {
                    duressSection.style.opacity = '';
                    duressSection.style.pointerEvents = '';
                    duressSection.title = '';
                    if (duressToggle) duressToggle.disabled = false;
                }
            }
        }
...
async function submitHiddenSpaceAction() {
    ...
    try {
        ...
        if (data.status === 'prompt_on_device') {
            showStatus(data.message || 'Check device screen to confirm with Space B PIN.', false);
        } else if (data.status === 'rebooting' || data.status === 'wiped_rebooting') {
...
document.getElementById('pincode-settings-form').addEventListener('submit', async function(e) {
    ...
    if (enabledForDevice !== wasEnabled) {
        ...
        if (enabledForDevice) {
            ...
            // Check if secondary slot exists — PIN disable is blocked entirely
            // while Hidden Space is provisioned, so warn and exit early instead
            // of proceeding to a confirm dialog that would just fail server-side.
            ...
            if (cachedHiddenSpace && cachedHiddenSpace.hidden_space_enabled) {
                ...
                showStatus('❌ Cannot disable PIN: Hidden Space is active. Remove Hidden Space first (Settings tab).', true);
                return;
            }
...
class SecureClient {
    ...
    shouldTunnelEndpoint(endpoint) {
        ...
        '/api/duress_pin_update', // ✅ POST - Duress PIN update
            ...
            '/api/enter_import_export_mode', // ✅ POST - вход в restricted import/export режим
    ...
    shouldSecureEndpoint(url) {
        ...
        '/api/duress_pin_update',  // 🔐 Duress PIN update (critical security)
            ...
            '/api/enter_import_export_mode',  // 🔐 Enter restricted import/export mode (critical!)
    ...
    async simpleAESEncrypt(plaintext) {
        ...
        try {
            ...
            // Split at bitArray level BEFORE encoding (tag is last 128 bits)
            const totalBits = sjcl.bitArray.bitLength(encrypted);
            const dataBits = sjcl.bitArray.clamp(encrypted, totalBits - 128);
            const tagBits = sjcl.bitArray.bitSlice(encrypted, totalBits - 128);
            
            // Encode each part to base64
            const dataBase64 = sjcl.codec.base64.fromBits(dataBits);
            const ivBase64 = sjcl.codec.base64.fromBits(ivBits);
            const tagBase64 = sjcl.codec.base64.fromBits(tagBits);
    ...
    async simpleAESDecrypt(base64Data, base64Iv, base64Tag) {
        try {
            const keyBits = sjcl.codec.hex.toBits(this.aesKey);
            
            // Decode base64 to bitArrays first
            const dataBits = sjcl.codec.base64.toBits(base64Data);
            const ivBits = sjcl.codec.base64.toBits(base64Iv);
            const tagBits = sjcl.codec.base64.toBits(base64Tag);
            
            // Concatenate at bitArray level (sjcl expects data+tag together)
            const combinedBits = sjcl.bitArray.concat(dataBits, tagBits);
...
function enterRestrictedImportExportMode() {
    document.getElementById('restricted-mode-password').value = '';
    document.getElementById('restricted-mode-modal').style.display = 'block';
}

function closeRestrictedModeModal() {
    document.getElementById('restricted-mode-modal').style.display = 'none';
}

function toggleRestrictedModePassword() {
    const input = document.getElementById('restricted-mode-password');
    input.type = input.type === 'password' ? 'text' : 'password';
}

async function submitEnterRestrictedMode() {
    const password = document.getElementById('restricted-mode-password').value;
    if (!password) {
        showStatus(tr('settings.restricted.err.password'), true);
        return;
    }
    try {
        const res = await makeEncryptedRequest('/api/enter_import_export_mode', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (data.status === 'rebooting') {
            closeRestrictedModeModal();
            showStatus(tr('settings.restricted.rebooting'), false);
        } else if (data.error) {
            showStatus(tr('settings.restricted.err.failed') + data.error, true);
        }
    } catch (err) {
        showStatus('Error: ' + err.message, true);
    }
...
document.querySelectorAll('.enter-restricted-mode-btn').forEach(button => {
    button.addEventListener('click', enterRestrictedImportExportMode);
});
